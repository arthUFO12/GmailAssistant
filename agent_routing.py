...
def make_router_prompt(state: dict):
    return f"""You are a router for a Gmail and Calendar chatbot. Your task is to route the workflow to the correct agent based on the current state of the current message log.
You should think step by step and decide carefully which to call. Ensure you reason in every response.
Respond JSON format with these fields:

thought: [Your reasoning]
action: [The structured tool call JSON objects]
agent: [The agent you are calling, (either gmail, calendar, or user)]
"""
@tool
@validate_call
...
general_model = ChatGoogleGenerativeAI(model='gemini-1.5-pro')
model = general_model.bind_tools(tools + [PromptUser, GiveUserInfo, CreateEvent, CreateTask, ConfirmRequestCompletion])
...
def routing_logic(state):
    ...
    if last_message.content == "gmail":
        return "gmail"
    elif last_message.content == "calendar":
        return "calendar"
    elif last_message.content == "user":
        return "user"
